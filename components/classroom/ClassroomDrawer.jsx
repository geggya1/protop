...
import { buildClassroomModules } from '../../src/navigation/classroomModules';
import { goPlatformOverview } from '../../src/utils/platformNav';
import { useApp } from '../../src/context/AppContext';
import { useI18n } from '../../src/i18n';
import { isClassroomAdmin, staffRoleOf } from '../../src/utils/classroom';

export default function ClassroomDrawer({
  visible, onClose, activeTab, onSelectTab, classroom, staff,
}) {
  const nav = useNavigation();
  const insets = useSafeAreaInsets();
  const { t } = useI18n();
  const { uid, familyId, selectFamily } = useApp();
  const isAdmin = isClassroomAdmin(classroom, uid, staff);
  const isStaff = !!staffRoleOf(classroom, uid, staff);

  const sections = useMemo(
    () => buildClassroomModules({ t, isAdmin, isStaff }),
    [t, isAdmin, isStaff],
  );
  ...
  const run = async (item) => {
    ...
    if (item.action?.type === 'nav') {
      ...
      const roomId = classroom?.id || familyId;
      params.classroomId = roomId;
      params.classroom = classroom;
      if (item.action.screen === 'GroupSettings' && roomId) {
        params.familyId = roomId;
        try { await selectFamily(roomId); } catch { /* fortsett likevel */ }
      }
      ...
      <Pressable style={styles.backdrop} onPress={onClose}>
        ...
        >
          <View style={styles.head}>
            <Text style={styles.headTitle} numberOfLines={1}>{classroom?.name || 'Klasserom'}</Text>
            <TouchableOpacity onPress={onClose} style={styles.closeBtn}>
              <Ionicons name="close" size={20} color={c.ink} />
            </TouchableOpacity>
          </View>
          {!!classroom?.school && (
            <Text style={styles.metaHint}>{classroom.school}{classroom.grade ? ` · ${classroom.grade}` : ''}</Text>
          )}
          {!!classroom?.joinCode && isAdmin && (
            <Text style={styles.codeHint}>Klassekode: {classroom.joinCode}</Text>
          )}
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                onClose?.();
                nav.navigate('ClassroomJoin');
              }}
            >
              <Ionicons name="key-outline" size={20} color={c.brand} />
              <Text style={styles.linkTxt}>Bli med med klassekode</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.linkRow}
              onPress={() => {
...
const styles = StyleSheet.create({
  ...
  metaHint: { color: c.tint, fontWeight: '700', fontSize: 13, marginBottom: 4 },
