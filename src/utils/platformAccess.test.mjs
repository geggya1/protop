...
assert.equal(canUsePlatformType('classroom', { email: DEV }), true);

assert.doesNotThrow(() => assertCanCreatePlatformType('family', OTHER));
...
assert.deepEqual(visibleGroupsForUser(groups, OTHER).map((g) => g.id), ['1']);
assert.deepEqual(visibleGroupsForUser(groups, DEV).map((g) => g.id), ['1', '2', '3']);
assert.deepEqual(visibleGroupsForUser(groups, OTHER, { isChild: true }).map((g) => g.id), ['1', '2', '3']);
